...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def validate_email(self, email):    #проверяет 
        if not User.objects.filter(email=email).exists():    #not False = True  #.exist(проверяет)
    ...
    def send_reset_password_code(self):
        ...
        user.create_activation_code()   #он генерирует новый пароль(код)
        user.save()
...
class ForgotPasswordCompleteSerializer(serializers.Serializer):
    ...
    def validate_code(self, code):
        if not User.objects.filter(activation_code=code).exists():
            raise serializers.ValidationError(f'{User} У вас неверный код')
        return code 

    def set_new_password(self):
        user = User.objects.get(activation_code=self.validated_data.get('code'))
        password = self.validate_data.get('password')
